...
export const ProjectDetails: React.FC<ProjectDetailsProps> = ({ activeProject }) => {
  ...
  return (
    <div className="w-full flex flex-col items-start justify-center gap-6 select-none font-mono">
      {/* Details Box with Glassmorphism (Expanded to max-w-xl) */}
      <div className="w-full max-w-xl border border-white/[0.04] bg-white/[0.01] hover:border-white/[0.08] hover:bg-white/[0.02] rounded-3xl p-6 md:p-8 backdrop-blur-md transition-all duration-300 relative overflow-hidden">
        ...
        <div className={`transition-all duration-300 ${fade ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          ...
          {/* Project Title and Number (Increased to text-2xl / text-3xl) */}
